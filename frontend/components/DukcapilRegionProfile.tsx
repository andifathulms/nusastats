...
import { dukcapilAncestry, formatNumber, type DukcapilRegionDetail } from "@/lib/api";
...
export function DukcapilRegionProfile({
  ...
  // Canonical ancestry (Kota Adm./Distrik/Kapanewon… aware), consistent with
  // the ranking/map labels rather than a raw title-case of nama_*.
  const scope = dukcapilAncestry(r.level, { domain_id: r.code, nama_prop: r.nama_prop, nama_kab: r.nama_kab, nama_kec: r.nama_kec });
  return (
    <Panel glow>
      <div className="mb-3 flex items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            {/* status may be comma-joined (Aceh: Daerah Istimewa, Daerah Khusus) -> one badge each */}
            {r.status
              ?.split(",")
              .map((s) => s.trim())
              .filter(Boolean)
