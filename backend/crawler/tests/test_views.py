...
def test_post_creates_crawl_run_and_enqueues_task(staff_client):
    with patch("crawler.views.run_incremental_crawl_task.delay") as mock_delay:
        resp = staff_client.post(
            ...
            {"subcat": "1", "max_subjects": "2", "max_variables": "5"},
        )

    assert resp.status_code == 302
    run = CrawlRun.objects.get()
    assert run.params == {"subcat": "1", "max_subjects": 2, "max_variables": 5}
    mock_delay.assert_called_once_with(run.id, subcat="1", max_subjects=2, max_variables=5)
...
def test_post_with_blank_fields_means_no_scoping(staff_client):
    ...
    run = CrawlRun.objects.get()
    assert run.params == {"subcat": None, "max_subjects": None, "max_variables": None}
    mock_delay.assert_called_once_with(run.id, subcat=None, max_subjects=None, max_variables=None)
